...
import { ROLES, SKILLS, AVAILABILITY, SEEDS, STEPS } from '../../data/teamData';
...
export const FounderProfileModal = ({ initialProfile, onSave, onClose }: Props) => {
  ...
  const [profile, setProfile] = useState<Partial<UserProfile>>({
    ...
    avatarSeed: initialProfile?.avatarSeed ?? SEEDS[Math.floor(Math.random() * SEEDS.length)],
  });
    ...
    >
      ...
      >
        ...
        <div className="p-8 pb-6 flex items-start justify-between">
          ...
          <button onClick={onClose} className="p-2 hover:bg-surface-container-low rounded-xl transition-all">
        ...
        <div className="px-8 mb-6">
          ...
          <p className="text-primary/40 text-xs font-body mt-2">{STEPS[step].subtitle}</p>
        </div>
        ...
        <div className="px-8 pb-8 flex items-center justify-between">
          ...
          >
            {step === STEPS.length - 1 ? 'Salva Profilo' : 'Continua'}
