      ...
      >
        <div className="bg-gradient-brand p-10 text-white relative overflow-hidden">
          ...
          <div className="flex items-center gap-8 relative z-10">
            <div className="relative shrink-0">
               ...
               {profile.badge && <div className="absolute -top-2 -right-2 bg-gradient-accent p-1.5 rounded-full border-4 border-white shadow-xl"><Zap className="w-4 h-4 text-white fill-current" /></div>}
            </div>
            <div>
        ...
        <div className="p-10 space-y-10 overflow-y-auto max-h-[60vh] scrollbar-hide">
          <section className="space-y-3">
            ...
            <div className="bg-surface-container-low/30 backdrop-blur-sm p-6 rounded-3xl border border-border-subtle/50 italic"><p className="text-primary/70 font-body leading-relaxed text-base">"{profile.vision}"</p></div>
          </section>
          <section className="space-y-3">
            <p className="text-[10px] text-secondary font-display font-black uppercase tracking-ultra">Competenze</p>
            <div className="flex flex-wrap gap-2">{profile.skills.map(skill => <span key={skill} className="px-4 py-2 bg-white border border-border-subtle/50 text-primary shadow-sm font-display font-black text-[10px] uppercase tracking-widest-plus rounded-xl">{skill}</span>)}</div>
          </section>
          <motion.button onClick={() => setSent(true)} disabled={sent} className={`w-full py-5 rounded-2xl font-display font-black text-xs uppercase tracking-widest-plus transition-all flex items-center justify-center gap-3 shadow-2xl relative overflow-hidden ${sent ? 'bg-secondary/10 text-secondary border border-secondary shadow-none cursor-default' : 'bg-gradient-brand text-white'}`}>{sent ? 'Richiesta Inviata' : 'Inizia Connessione'}</motion.button>
        </div>
      </motion.div>
    </motion.div>
  );
      ...
      >
        <div className="bg-gradient-brand p-8 md:p-10 text-white relative shrink-0">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-8 relative z-10">
            <div className="w-20 h-20 bg-white/10 backdrop-blur-xl rounded-2xl flex items-center justify-center border border-white/20 shadow-inner"><Building2 className="w-10 h-10 text-tertiary" /></div>
            <div>
              <div className="flex items-center gap-3 mb-2"><span className="text-[10px] font-display font-black uppercase tracking-mega text-tertiary/80">{coop.sector}</span><div className="w-1 h-1 rounded-full bg-tertiary/40" /><div className="flex items-center gap-2"><MapPin size={10} className="text-tertiary" /><span className="text-[10px] font-display font-black uppercase text-white/70">{coop.location}</span></div></div>
        ...
        <div className="p-8 md:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10 overflow-y-auto scrollbar-hide">
          <div className="lg:col-span-7 space-y-10">
            <section className="space-y-3"><div className="flex items-center gap-3 text-secondary"><LayoutGrid size={16} /><h5 className="text-[10px] font-display font-black uppercase tracking-ultra">Identità</h5></div><p className="text-primary/70 font-body leading-relaxed text-sm italic">"{coop.description}"</p></section>
            <section className="space-y-3"><div className="flex items-center gap-3 text-secondary"><Award size={16} /><h5 className="text-[10px] font-display font-black uppercase tracking-ultra">Missione</h5></div><div className="bg-surface-container-low/30 backdrop-blur-sm p-6 rounded-3xl border border-border-subtle/50 relative overflow-hidden group"><p className="text-primary/60 font-body text-base leading-relaxed relative z-10">{coop.mission}</p></div></section>
          </div>
          <div className="lg:col-span-5 space-y-6">
            {coop.studioInCampoAvailable && (
              <div className="bg-gradient-accent p-6 rounded-4xl text-white relative overflow-hidden group shadow-xl shadow-secondary/20"><Zap className="absolute -top-3 -right-3 w-20 h-20 text-white/10" /><div className="flex items-center gap-3 mb-4"><div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-md border border-white/20 font-display text-white italic font-black text-lg">S</div><h6 className="font-display font-black text-white text-base leading-none">Studio in Campo</h6></div><p className="text-[11px] font-body text-white/80 leading-relaxed mb-6">Osserva i processi operativi di {coop.name} dal vivo.</p><button onClick={() => setRequestSent(true)} disabled={requestSent} className={`w-full py-4 rounded-xl font-display font-black text-[10px] uppercase tracking-widest-plus transition-all ${requestSent ? 'bg-white/20' : 'bg-white text-primary'}`}>{requestSent ? 'Richiesta Inviata' : 'Prenota Posto'}</button></div>
            )}
            <div className="bg-white/50 backdrop-blur-md p-6 rounded-4xl border border-white/50 space-y-4">
               <h5 className="text-[9px] font-display font-black uppercase tracking-ultra text-primary/40">Collaborazione</h5><p className="text-[11px] font-body text-primary/60 leading-relaxed">Contatta il board di {coop.name}.</p><button className="w-full py-3.5 bg-white border border-border-subtle rounded-xl font-display font-black text-[9px] uppercase tracking-widest-plus transition-all text-primary shadow-sm hover:bg-primary hover:text-white">Presenta Visione <Share2 size={12} /></button>
            </div>
          </div>
        </div>
      </motion.div>
