        ...
        {!feedback || !feedback.isCorrect ? (
            ...
            <div className="grid gap-4">
              {data.options.map((opt, i) => (
                <motion.button
                  ...
                  className="w-full p-6 text-left rounded-3xl bg-white/50 hover:bg-white border border-white/50 hover:border-primary/30 transition-all text-text-primary font-body text-base font-medium flex items-center justify-between group shadow-sm hover:shadow-xl"
                >
          ...
          <motion.div key="f" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10">
             <div className="w-24 h-24 bg-accent-warm/10 rounded-4xl flex items-center justify-center mx-auto mb-8 border border-accent-warm/20 shadow-glow shadow-accent-warm/20">
                <Zap className="text-accent-warm w-10 h-10 fill-accent-warm" />
             </div>
